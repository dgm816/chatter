...
int irc_recv(Irc *irc, char *buf, int size, bool *needs_refresh, char *out_command_buf, int out_command_buf_size) {
    ...
    *needs_refresh = false; // Initialize output parameter
    ...
    // We try to read up to `size - 1` bytes, so we need at least that much space.
    // If current capacity + `size` is less than `recv_buffer_len + size`, reallocate.
    if (irc->recv_buffer_len + size >= irc->recv_buffer_capacity) {
        size_t new_capacity = irc->recv_buffer_capacity;
        while (irc->recv_buffer_len + size >= new_capacity) {
            new_capacity *= 2;
        }
    ...
    // Read data into the buffer after existing data
    if (irc->ssl) {
        bytes_received = SSL_read(irc->ssl, irc->recv_buffer + irc->recv_buffer_len, size - 1);
    } else {
        bytes_received = recv(irc->sock, irc->recv_buffer + irc->recv_buffer_len, size - 1, 0);
    }

    if (bytes_received > 0) {
        irc->recv_buffer_len += bytes_received;
        irc->recv_buffer[irc->recv_buffer_len] = '\0'; // Null-terminate the accumulated buffer
        log_message("RECV (accumulated): %s", irc->recv_buffer);
